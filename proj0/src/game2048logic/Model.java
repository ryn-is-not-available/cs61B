...
public class Model {
    ...
    public boolean emptySpaceExists() {
        ...
        for(int x=0;x<len;x++){
            for(int y=0;y<len;y++){
                if(board.tile(x,y)==null) return true;
            }
        }
        return false;
    }
        ...
        for(int x=0;x<len;x++){
            for(int y=0;y<len;y++){
                ...
                for(int i=0;i<4;i++){
                        int newX = x + movesX[i];
                        int newY = y + movesY[i];
                        if(0<=newX && newX<len && 0<=newY && newY<len){
                            Tile adjTile = board.tile(newX,newY);
                            if(adjTile==null)return true;
                            if(adjTile.value()==tmp.value())return true;
                        }
                    }
    ...
    public void moveTileUpAsFarAsPossible(int x, int y) {
        ...
        if (currTile != null) {
            ...
            int len = board.size();
            while (targetY + 1 < len) {
                Tile tmp = board.tile(x, targetY + 1);
                if (tmp == null) {
                    targetY++;
                } else if (tmp.value() == currTile.value()) {
                    if (tmp.wasMerged()) break;
                    targetY++;
                    score += tmp.value() * 2;
                    break;
                } else break;
